            ...
            return {
                ...
                rules: {
                    user_eno: [
                        ...
                        {min: 10, max: 10, message: '长度为十位数字', trigger: 'blur'}
                    ],
                    user_name: [
                        {required: true, message: '请输入科研人员姓名', trigger: 'blur'},
                        {min: 2, max: 24, message: '长度在 2 到 12 个字符', trigger: 'blur'}
                    ],
                    user_pwd: [
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8181/userManagerPersonnel/save', this.ruleForm).then(function (resp) {
                            // console.log(resp);
